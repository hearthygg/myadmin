...
function hasOneShowingChild(children = [] as any, parent: any) {
  ...
  if (showingChildren.length === 1) {
    // console.log(showingChildren[0],JSON.parse(JSON.stringify(onlyOneChild.value)))
    return true;
  }
...
<template>
  ...
  <div v-if="!item.meta || !item.meta.hidden">
    <!-- 下面v-if要加上 && (!item.meta || !item.meta.alwaysShow) -->
    <template v-if="hasOneShowingChild(item.children, item) && (!onlyOneChild.children || onlyOneChild.noShowingChildren)">
      <app-link v-if="onlyOneChild.meta" :to="resolvePath(onlyOneChild.path)">
        <el-menu-item :index="resolvePath(onlyOneChild.path)" :class="{ 'submenu-title-noDropdown': !isNest }">
          ...
          <template #title>
            <span :class="{ 'ml-2': settingsStore.layout === 'top' }">{{ onlyOneChild.meta.title }}</span>
          </template>
        </el-menu-item>
      </app-link>
    </template>
